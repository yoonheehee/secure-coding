...
def main():
        ...
        with col2:
            ...
            if st.button('Sign Up'):
                response = requests.get('http://localhost:8000/register',
                                        params={"username": new_username, "password": new_password, "role": "user",
    ...
    if st.session_state.logged_in:
        if st.session_state.user["role"] == 'admin':
            ...
            menu = ['Home', 'All Purchase History', 'Add Product']
            ...
            if choice == 'Home':
            ...
            elif choice == 'All Purchase History':
                st.subheader('All Purchase History')
                response = requests.get('http://localhost:8000/get_all_purchase',
                                        params={"username": st.session_state.user["username"]})
                purchases = response.json()
                for purchase in purchases:
                    st.write(f"Product Name: {purchase['product_name']}, Payment Complete: {'true' if purchase['payment_complete'] else 'false'}, Purchase Time: {purchase['purchase_time']}, Price: ${purchase['product_price']}, Buyer Address: {purchase['buyer_address']}, Buyer Buyer Username: {purchase['buyer_address']}")
                    if 'product_thumbnail_url' in purchase and purchase['product_thumbnail_url'] != '':
                        st.image(purchase['product_thumbnail_url'], width=200)
                ...
                with st.form(key='add_product_form'):
                    ...
                    if submit_button:
                        add_product_response = requests.get('http://localhost:8000/add_product',
            ...
            menu = ['Home', 'Buy Products', 'My Purchase History', 'My Page']
            choice = st.sidebar.selectbox('Menu', menu)
            ...
            if choice == 'Home':
                ...
                if st.button('Buy'):
                    ...
                    purchase_response = requests.get('http://localhost:8000/add_purchase',
                                            params={"username": st.session_state.user["username"],
                    ...
                    if purchase_response.status_code == 200:
                        st.success(
                            f'You have successfully purchased {selected_product}! Address: {user_address}, Payment Info: {user_payment_info}')
                    else:
                        st.error("Failed to add purchase.")

            elif choice == 'My Purchase History':
                st.subheader('My Purchase History')
                response = requests.get('http://localhost:8000/get_my_purchase',
                                        params={"username": st.session_state.user["username"]})
                purchases = response.json()
                for purchase in purchases:
                    st.write(f"Product Name: {purchase['product_name']}, Payment Complete: {'true' if purchase['payment_complete'] else 'false'}, Purchase Time: {purchase['purchase_time']}, Price: ${purchase['product_price']}, Buyer Address: {purchase['buyer_address']}")
                    if 'product_thumbnail_url' in purchase and purchase['product_thumbnail_url'] != '':
                        st.image(purchase['product_thumbnail_url'], width=200)
                ...
                with st.form(key='edit_user_info_form'):
                    ...
                    if submit_button:
                        response = requests.get('http://localhost:8000/update_user_info',
                                                params={"username": new_username, "full_name": new_full_name,
                                                        "address": new_address, "payment_info": new_payment_info})
                        if response.status_code == 200:
